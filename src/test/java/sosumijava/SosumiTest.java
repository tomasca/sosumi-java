...
public class SosumiTest {
    ...
    public void setUp() throws Exception {
        ...
        primaryServiceAddress = "http://" + server.getServiceAddress().getHostName() + ":" + server.getServiceAddress().getPort();
        secondaryServiceAddress = server.getServiceAddress().getAddress().getHostAddress() + ":" + server.getServiceAddress().getPort();
        
        impl = new Sosumi(primaryServiceAddress, "user", "pass");
    ...
    public void testRefresh() throws Exception {
        ...
        doAnswer(new Answer() {
            ...
            // Second request is a 200 OK containing a json payload
            public Object answer(InvocationOnMock invocation) throws Throwable {
                Object[] args = invocation.getArguments();
                HttpRequest req = (HttpRequest) args[0];
                HttpResponse resp = (HttpResponse) args[1];
                HttpContext ctx = (HttpContext) args[2];
                //assertEquals("should use new hostname", secondaryServiceAddress)
                assertTrue("should be initClient command and URL shall include 'scope'", req.getRequestLine().getUri().endsWith(scope + "/initClient"));
        ...
        assertEquals(secondaryServiceAddress, impl.getPartition().getHost() + ":" + impl.getPartition().getPort());
    }
